...
// Thread 1: Ingestion Pipeline (UDP Multicast Consumer)
void network_ingestion_loop() {
    aerohedge::UdpListener listener("239.255.0.1", 9000);
    listener.listen_and_publish(market_queue); 
}

// Thread 2: Risk Optimization Loop (Math Engine)
void risk_execution_loop() {
    // Strike: 100, TTE: 30 days, Rate: 5%, Vol: 20%, Hedging Margin: 5 shares
    aerohedge::RiskEngine engine(100.0, 0.082, 0.05, 0.20, 5.0, outbound_queue);
...
// Thread 3: Transmission Gateway Loop (TCP Non-Blocking Producer)
...
int main() {
    ...
    // Isolate hardware resource footprints across system physical cores
