...
private:
    ...
    uint64_t latency_records_[10000]; // Pre-allocated storage for 10,000 trades
    int trade_count_ = 0; 

    // Reference to the lock-free pipeline out to the network gateway
...
public:
    RiskEngine(double k, double tte, double r, double vol, double thresh, SPSCQueue<OrderRequest, 1024>& out_q) 
        : strike_(k), time_to_expiry_(tte), risk_free_rate_(r), volatility_(vol), 
          current_position_(0), delta_threshold_(thresh), order_id_counter_(1), outbound_queue_(out_q) {}
    ...
    void process_tick(const MarketTick& tick) {
        ...
        if (std::abs(hedge_diff) >= delta_threshold_) {
            ...
            double latency_ns = global_clock.cycles_to_ns(cycle_diff);
            
            // Store latency for reporting (avoiding slow std::vector push_back)
            if (trade_count_ < 10000) {
                latency_records_[trade_count_++] = latency_ns;
            }
